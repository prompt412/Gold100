...
pii next(int d, int b, int x, int y) {
  switch (d) {
    case 0:
      return b ? pii{x-1, y+1} : pii{x+1, y+1};
    case 1:
      return b ? pii{x, y+1} : pii{x+1, y-1};
    case 2:
      return b ? pii{x+1, y+1} : pii{x, y-1};
    case 3:
      return b ? pii{x+1, y-1} : pii{x-1, y-1};
    case 4:
      return b ? pii{x, y-1} : pii{x-1, y+1};
...
auto h = [](int x, int y) { return x * 50 + y; };

int dfs(int x, int y, int rot, int rem, ivec& us) {
  if (rem == 0) {
    return int(us[h(x, y)] > 0);
  }
  if (us[h(x, y)] > 0) {
    return 0;
  }
  us[h(x, y)] = 1;
  int ret = 0;
  auto [x1, y1] = next(rot, 0, x, y);
  ret += dfs(x1, y1, (rot+1)%6, rem-1, us);
  auto [x2, y2] = next(rot, 1, x, y);
  ret += dfs(x2, y2, (rot+5)%6, rem-1, us);
  us[h(x, y)] = 0;
  return ret;
}
